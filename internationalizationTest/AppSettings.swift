...
class AppSettings: NSObject,NSCoding,NSSecureCoding {
    func encode(with coder: NSCoder) {
        coder.encode(self.language.rawValue, forKey: "language")
    }
    
    required init?(coder: NSCoder) {
        let str = coder.decodeObject(forKey: "language") as! String
        self.language = AppSettings.Language(rawValue: str)!
    }
    ...
    static let shared: AppSettings = {
        ...
        if let savedData = UserDefaults.standard.object(forKey: AppSettings.kSharedSettingsKey) as? Data,
            ...
            appSettings = AppSettings.init(language: .Chinese)
        }
